...
public class SuperviseInfoMainController {
    ...
    public ResponseEntity<Result> saveSuperviseInfoCheck(@RequestBody @Valid List<SuperviseType> superviseTypeList) {
        log.info("保存监管业务（主表-未整改未监管项）：{}", superviseTypeList);
        Result result = this.superviseInfoMainService.saveSuperviseInfoCheck(superviseTypeList);
        return ResponseEntity.ok(result);
    }
