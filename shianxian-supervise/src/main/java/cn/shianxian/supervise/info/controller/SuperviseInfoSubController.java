...
public class SuperviseInfoSubController {
     ...
     */
    @GetMapping("rectify")
    @ApiOperation(value = "查询整改情况", notes = "查询整改情况")
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "query", name = "mainId", value = "监管业务编码"),
            @ApiImplicitParam(paramType = "query", name = "nodeTag", value = "企业编码"),
            @ApiImplicitParam(paramType = "query", name = "superviseTypeTag", value = "监管类型编码"),
    })
    public ResponseEntity<Result> rectify(SuperviseInfoSub superviseInfoSub) {
        Result result = this.superviseInfoSubService.rectify(superviseInfoSub);
        return ResponseEntity.ok(result);
    }
